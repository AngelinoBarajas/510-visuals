...
decodeDots(DOTS_B64).then(function(LAND_DOTS){
  ...
  function createPin(loc,idx){
    ...
    // Bigger core dot
    var coreR=isHQ?0.014:isHero?0.012:0.009;
    var core=new THREE.Mesh(new THREE.CircleGeometry(coreR,24),new THREE.MeshBasicMaterial({color:isHQ?0xd0e0e3:isHero?0xb8c9cc:0x8aacb2,side:THREE.DoubleSide}));
    core.position.copy(pos);core.lookAt(pos.clone().multiplyScalar(2));pinGroup.add(core);

    // Hitbox
    var hitbox=new THREE.Mesh(new THREE.SphereGeometry(isHQ?0.05:isHero?0.045:0.04,12,12),new THREE.MeshBasicMaterial({visible:false}));
    ...
    var beamPts=[pos.clone(),beamEnd];
    var beamGeo=new THREE.BufferGeometry().setFromPoints(beamPts);
    var beam=new THREE.Line(beamGeo,new THREE.LineBasicMaterial({color:isHQ?0xd0e0e3:isHero?0xb8c9cc:0x8aacb2,transparent:true,opacity:isHQ?0.5:0.3}));
    pinGroup.add(beam);
